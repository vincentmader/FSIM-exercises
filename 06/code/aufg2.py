...
=======
    ...
    # apply Laplace in fourierspace
>>>>>>> a3387ee184c63a510d74ab74b7f7180d066efdbf
...
def initialize_particles(r_min, r_max, N=100):
    ...
    testpart[:, 0] = pos[0] + r_min * \
        (r_max/r_min) ** randNo[:, 0] * np.sin(2 * np.pi*randNo[:, 1])
    testpart[:, 1] = pos[1] + r_min * \
        (r_max/r_min) ** randNo[:, 0] * np.cos(2 * np.pi*randNo[:, 1])
