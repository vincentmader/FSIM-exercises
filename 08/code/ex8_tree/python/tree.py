...
class TreeClass:
    def __init__(self, particles):
        ...
        # here we make a list of interactions per particle
        self.interactions = np.zeros(len(particles))
        ...
        for iz in range(0, 2):
            for iy in range(0, 2):
                for ix in range(0, 2):
                    ...
                    self.nodelist[childid].xc[0] = self.nodelist[nodeid].xc[0] + \
                        0.25*size*(2*ix-1)
                    self.nodelist[childid].xc[1] = self.nodelist[nodeid].xc[1] + \
                        0.25*size*(2*iy-1)
                    self.nodelist[childid].xc[2] = self.nodelist[nodeid].xc[2] + \
                        0.25*size*(2*iz-1)
    ...
    def insertparticle(self, nodeid, pid):
        if self.nodelist[nodeid].particleid >= 0:
            ...
            if origpid == pid:
                print(
                    'ERROR: Inserting particle that is already inserted: ' +
                    str(pid)+' '+str(origpid)
                )
                quit()
    ...
    def gforce(self, nodeid, pid, anglemax):
        ...
        if angle < anglemax or self.nodelist[nodeid].leaf == 1:
            ...
            # factor 8 because 8 subnotes get called each time.
            self.interactions[pid] += 8
            for iz in range(0, 2):
                for iy in range(0, 2):
                    for ix in range(0, 2):
