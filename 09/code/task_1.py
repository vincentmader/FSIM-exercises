...
def plot(q, task, scheme='upwind', ts=[0, 100], xlims=None):
    ...
    for idx, t in enumerate(ts):
        label = 3 if idx else 0
        plt.plot(x, q[t], label=f"t={label}", color=colors[idx])
...
plot(q, '6_0', ts=[0, 100], xlims=xlims)
q = advection(q_0, t_N=1000)
plot(q, 6, ts=[0, 1000], xlims=xlims)
q = advection(q_0, t_N=10)
plot(q, 7, ts=[0, 10], xlims=xlims)
